...
if __name__ == "__main__":
    ...
    for item in administrative_types:
        ...
        skip_pl = skip_en = False

        wb_update = wbi_core.ItemEngine(item_id=item)
        label_pl = wb_update.get_label('pl')
        if label_pl not in nazwy_pl:
            skip_pl = True

        label_en = wb_update.get_label('en')
        if label_en not in nazwy_en:
            skip_en = True

        if skip_pl and skip_en:
            continue
        ...
        for statement in wb_update.statements:
            ...
            if prop_nr == p_part_of:
                ...
                wb_test = wbi_core.ItemEngine(item_id=statement_value)
                for t_statement in wb_test.statements:
                    t_prop_nr = t_statement.get_prop_nr()
                    if t_prop_nr == p_instance_of:
                        t_statement_value = t_statement.get_value()
                        t_statement_type = t_statement.data_type
                        t_statement_value = statement_value_fix(t_statement_value, t_statement_type)
                        if t_statement_value == q_administrative_system:
                            adm_system_pl = wb_test.get_label('pl')
                            adm_system_en = wb_test.get_label('en')
                            break
                    if adm_system_pl or adm_system_en:
                        break

        if adm_system_pl:
            if not skip_pl:
                label_pl = f"{label_pl} ({adm_system_pl})"
                if WIKIBASE_WRITE:
                    wb_update.set_label(label_pl, 'pl')
            if not skip_en:
                label_en = f"{label_en} ({adm_system_en})"
                if WIKIBASE_WRITE:
                    wb_update.set_label(label_en, 'en')

            if WIKIBASE_WRITE:
                wb_update.write(login_instance, entity_type='item')

            print(f"{item}: {label_pl}) / {label_en}")
