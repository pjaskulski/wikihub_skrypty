""" import słownika SHG Liw - wymaga wikibaseintegrator 0.12 lub nowszej """
...
from wikibaseintegrator.datatypes import MonolingualText, String, Item, Time
...
if __name__ == '__main__':
    ...
    for row in ws.iter_rows(start_row, max_row):
        ...
        if shg_id is None or ahp_ok is None or ahp_ok == '' or ahp_ok == 'None' or ahp_ok in ('NEW', 'XXX', '???'):
        ...
        if 'prng:' in ahp_ok:
            ok, qid = search_by_unique_id(prop_id=P_PRNG, id_value=ahp_ok.replace('prng:','').strip())
        else:
            ok, qid = search_by_unique_id(prop_id=P_AHP_ID, id_value=ahp_ok)

        if ok:
            wb_item = wbi.item.get(entity_id=qid)
        else:
            logger.error(f'Nie znaleziono miejscowości z AHP ID = {ahp_ok}')
            sys.exit(1)

        print(f'{nazwa} ({ahp_ok}) https://prunus-208.man.poznan.pl/wiki/Item:{qid}')
