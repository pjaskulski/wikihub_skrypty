""" import jednostek administracyjnych innych z pliku PRNG_egzonimy_JA2.xlsx """
import os
import sys
import time
...
if __name__ == '__main__':
    ...
    xlsx_input = '../data_prng/PRNG_egzonimy_JAi.xlsx'
    wb = openpyxl.load_workbook(xlsx_input)
    ws = wb["PRNG_egzonimy_JAi"]
    ...
    for index, row in enumerate(ws.iter_rows(2, ws.max_row), start=1):
        ...
        description_pl = 'jednostka administracyjna inna'
        description_en = 'other administrative unit'
        ...
        # instance of
        if inform_dod and 'obszar separatystyczny' in inform_dod:
            instance_of_value = q_other_territories
        else:
            instance_of_value = q_administrative_unit

        statement = create_statement_data(p_instance_of, instance_of_value, None,
                                          qualifiers,
                                          add_ref_dict=None)
        if statement:
        ...
        statement = create_statement_data(p_stated_as, f'pl:"{nazwa}"', None,
                                          local_qualifiers,
                                          add_ref_dict=references)
        if statement:
            data.append(statement)
        ...
        if nazwa_dlug and nazwa_dlug != nazwa:
            ...
            statement = create_statement_data(p_stated_as, f'pl:"{nazwa_dlug}"', None,
                                              local_qualifiers,
                                              add_ref_dict=references)
            if statement:
                data.append(statement)
        ...
        if nazwa_obocz and nazwa_obocz not in (nazwa, nazwa_dlug):
            ...
            statement = create_statement_data(p_stated_as, f'pl:"{nazwa_obocz}"', None,
                                              local_qualifiers,
                                              add_ref_dict=references)
            if statement:
                data.append(statement)
